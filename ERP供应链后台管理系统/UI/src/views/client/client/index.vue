  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="联系人" prop="clientPerson">
        <el-input
          v-model="queryParams.clientPerson"
          placeholder="请输入联系人"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="clientList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分类ID" align="center" prop="clientClassifyId" />
        ...
        <template slot-scope="scope">
           <span v-if="scope.row.auditStatus === 1">
              <el-button
                ...
                v-hasPermi="['client:client:approver']"
              >审批</el-button>
           </span>
          ...
          >删除</el-button>
            <el-button
              ...
              v-hasPermi="['client:client:audit']"
            >审核</el-button>
          </span>
    ...
    <el-dialog :title="title" :visible.sync="open" width="960px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="选择客户分类">
          <el-select v-model="form.clientClassifyId"  placeholder="请选择客户分类">
            <el-option v-for="item in clientClassifyList"
                       :key="item.clientClassifyName"
                       :label="item.clientClassifyName"
                       :value="item.clientClassifyId"
            >
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="联系人" prop="clientPerson">
          <el-input v-model="form.clientPerson" placeholder="请输入联系人" />
        </el-form-item>
        <el-form-item label="联系电话" prop="clientPhone">
...
<script>
import { listClient, getClient, delClient, addClient, updateClient,auditClient,approverClient } from "@/api/client/client";
import {listClientclassify} from "../../../api/clientclassify/clientclassify";
...
export default {
  ...
  methods: {
    //审批提交数据
    submitFormApprover(){
      approverClient(this.formApprover).then(response => {
    ...
    //审批窗口
    handleApprover(row){
      this.formApprover.clientId = row.clientId
      this.openApprover = true
      this.title = '审批窗口'
    },
    //审核提交数据
    submitFormAudit(){
      auditClient(this.formAudit).then(response => {
        this.$modal.msgSuccess("审核操作成功");
    ...
    //审核窗口
    handleAudit(row){
      this.formAudit.clientId = row.clientId
      this.openAudit = true
      this.title = '审核窗口'
    },
    //查询所有分类
    clientClassifyLists(){
      this.loading = true;
      listClientclassify(null).then(response => {
        this.clientClassifyList = response.rows;
        this.loading = false;
      });
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除客户列表编号为"' + clientIds + '"的数据项？').then(function() {
